...
class CongViecRepository {
  ...
  Future<CongViecModel> getCongViecById(String id) async {
    try {
      final String? token = prefs.getString('token');
      if (token == null) throw Exception('Token không tồn tại');

      final url = Uri.parse('${dotenv.env['API_URL']}/api/CongViec/$id');
      final headers = {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer $token',
      };
  ...
  Future<CongViecConModel> insertCVC(CongViecConModel cvc) async {
    ...
    try {
      final response = await http.post(
        Uri.parse(
          '${dotenv.env['API_URL']}/api/congviec/InsertCVC'
          '?userName=${cvc.createBy}',
        ),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
      ...
      final apiResponse = ApiResponseModel.fromJson(json.decode(response.body));
      if (!apiResponse.success || apiResponse.data == null) {
        throw Exception(apiResponse.message);
      }

      return CongViecConModel.fromJson(
  ...
  Future<CongViecConModel> updateCVC(CongViecConModel cvc) async {
    final token = prefs.getString('token');
    if (token == null) throw Exception('Token không tồn tại');
    ...
    try {
      final response = await http
          .put(
            Uri.parse(
              '${dotenv.env['API_URL']}/api/congviec/UpdateCVC'
  ...
  Future<bool> deleteCVC(String id, String userName) async {
    final token = prefs.getString('token');
    if (token == null) throw Exception('Token không tồn tại');
    ...
    try {
      final response = await http.delete(
        Uri.parse(
          '${dotenv.env['API_URL']}/api/congviec/DeleteByIdCVC/$id'
          '?userName=$userName',
        ),
        headers: {
          'Content-Type': 'application/json',
